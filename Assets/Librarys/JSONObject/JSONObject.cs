...
namespace UnityEngine
{
	...
	public float f {
        ...
        public static JSONObject nullJO => Create(Type.NULL); //an empty, null object
        public static JSONObject obj => Create(Type.OBJECT); //an empty object
        public static JSONObject arr => Create(Type.ARRAY); //an empty array
	...
	public JSONObject(double d) {
        ...
        /// <param name="maxDepth">
        ///     The maximum depth for the parser to search.  Set this to to 1 for the first level,
        ///     2 for the first 2 levels, etc.  It defaults to -2 because -1 is the depth value that is parsed (see below)
        /// </param>
        /// <param name="storeExcessLevels">Whether to store levels beyond maxDepth in baked JSONObjects</param>
        /// <param name="strict">
        ///     Whether to be strict in the parsing. For example, non-strict parsing will successfully
        ///     parse "a string" into a string-type
        /// </param>
        /// <returns></returns>
                    ...
                    else
                    {
                        ...
                        while (++offset < str.Length)
                        {
                            ...
                            if (str[offset] == '[' || str[offset] == '{')
                                depth++;
                            else if (str[offset] == ']' || str[offset] == '}') depth--;
                            //if  (encounter a ',' at top level)  || a closing ]/}
                            if ((str[offset] == ',' && depth == 0) || depth < 0)
                            {
                                inProp = false;
                                var inner = str.Substring(tokenTmp, offset - tokenTmp).Trim(WHITESPACE);
                                if (inner.Length > 0)
                                {
                                    if (type == Type.OBJECT)
                                        keys.Add(propName);
...
#pragma warning disable 219
        ...
        public IEnumerable<string> PrintAsync(bool pretty)
        {
            ...
            printWatch.Reset();
            printWatch.Start();
            foreach (IEnumerable e in StringifyAsync(0, builder, pretty)) yield return null;
